...
class RoverCamera extends THREE.PerspectiveCamera {
    ...
    goToLatLong(lat, long) { // go to position on earth
        ...
        lat -= this.ground.geometry.latIdx;
        long -= this.ground.geometry.longIdx;
        
        // ! potential outOfBounds below
        var xIdx = Math.floor(long * 1201);
        var yIdx = Math.floor((1 - lat) * 1201);
        var idx = yIdx * 1201 + xIdx;
        ...
        for(var y = -heightHlf; y <= heightHlf; y++) { // cut a part out from the geometry
            for(var x = -widthHlf; x <= widthHlf; x++) {
                var modIdx = idx + y * 1201 + x;
                
                if(modIdx < 0 || modIdx > 1201*1201) {continue;} // if out of bounds, skip
